...
""" 모든 예보는 현재시간을 기준으로 24시간 안에서만 조회 가능합니다. """
# #현재 시점을 기준으로 최신 예보정보를 조회하겠습니다. 
time = datetime.now()
#time = datetime.strptime("2020-12-15 03:02:23","%Y-%m-%d %H:%M:%S")
print(f"[Time: {time:%Y-%m-%d %H:%M}]")
...
#version  = KMA.getFcstVersion("SHRT", baseTime, save_path = "./file_name_version.xml") 
...
#df_VilageFcst = KMA.getVilageFcst(X, Y, time, save_path="./file_name_data.xml")

# #데이터프레임을 CSV파일로 저장하고 싶으면 [ .to_csv("./file_name.csv") ] 를 붙이세요.
df_VilageFcst.to_csv("./file_name_VilageFcst.csv", encoding='utf-8')
...
df_UltraSrtFcst.to_csv("./file_name_UltraSrtFcst.csv", encoding='utf-8')
...
df_UltraSrtNcst.to_csv("./file_name_UltraSrtNcst.csv", encoding='utf-8')
# #==========================================================================================